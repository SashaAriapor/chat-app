...
const { updateUserProfile, checkUsernameIsAvailable, findByUsername } = require("../../services/user/user.service");
const fs = require("fs");
...
module.exports = new class UserController {
    ...
    async editUsername(req, res, next) {
        try {
            ...
            const result = await checkUsernameIsAvailable(username);
            if (result) {
                const data = { username }
                const user = await updateUserProfile({phoneNumber}, data);
                return res.status(StatusCodes.ACCEPTED).json({
